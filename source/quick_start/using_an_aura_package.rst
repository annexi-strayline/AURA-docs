...
To do that, we'll use a third-party subsystem package called "CLI", from ANNEXI-STRAYLINE's  `ASAP <https://github.com/annexi-strayline/ASAP>`_ repository.
...
Lets fix this by adding the `ASAP <https://github.com/annexi-strayline/ASAP>`_ repository.
...
Repositories are defined by simple Ada package specifications. :ref:`Repository 1 <ref-repository-1>` is a special repository that is automatically generated by the AURA CLI and basically refers to the project itself. Therefore we will need to create a repository configuration for **Repository 2**.
