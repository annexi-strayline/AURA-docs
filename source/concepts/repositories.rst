...
#. If a checkout package does not exist for the subsystem, it shall be created if the subsystem is located within any of the configured repositories. The checkout package must be a renaming of the source repository declaration.
...
#. After a successful checkout, the subsystem shall be configured, and then entered into the project. Configuration may cause additional codepaths to be activated. After entering the configured subsystem, any further subsystem dependencies identified are checked-out as needed.
